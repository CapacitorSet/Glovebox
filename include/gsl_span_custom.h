...
template <class Span, bool IsConst> class span_iterator {
  ...
  public:
	...
	*/
	constexpr std::conditional_t<IsConst, const span<element_type_, 1>,
	                             span<element_type_, 1>>
	operator*() const {
		Expects(index_ != span_->size());
		return std::conditional_t<IsConst, const span<element_type_, 1>,
		                          span<element_type_, 1>>(
		    span_->subspan(index_, 1), 0);
	}
	...
	constexpr pointer operator[](difference_type n) const {
		return *(*this + n);
	}
...
template <class ElementType, std::ptrdiff_t Extent> class span {
  public:
	...
	template <
	    ...
	    class = std::enable_if_t<
	        Extent == dynamic_extent && // Do not allow conversions from span<T>
	                                    // to span<T, n>
	...
	// Actually, allow them just for internal usage, since we need them to
	// return span<T, 1> for operator[].
	template <
	    class OtherElementType, std::ptrdiff_t OtherExtent,
	    class = std::enable_if_t<
	        details::is_allowed_extent_conversion<OtherExtent, Extent>::value &&
	        details::is_allowed_element_type_conversion<OtherElementType,
	                                                    element_type>::value>>
	constexpr span(const span<OtherElementType, OtherExtent> &other,
	               char this_constructor_is_for_internal_usage)
	    : storage_(other.data(),
	               details::extent_type<OtherExtent>(other.size())) {
	...
	constexpr pointer operator()(index_type idx) const {
		return this->operator[](idx);
	}
	constexpr pointer data() const noexcept { return storage_.data(); }
	constexpr element_type *cptr() const noexcept {
  ...
  private:
	...
	constexpr pointer data_plus(index_type offset) const {
		...
		return pointer(data(), data().get() +
		                           offset); // Uses same ref counter as data()
	}
};
