...
TEST_F(ArrayTest, ReadWriteEncrypted) {
	::rc::detail::checkGTest([=](int8_t plaintext_num, uint8_t index) {
		Int8 index_int = Int8(index % 8, params);
		auto arr = Array<Int8, 8>(true, params);
		auto elem = Int8(plaintext_num, params);
		arr.put(elem, index_int);

		auto out =
		    Int8(0, params); // Initialize memory so ubsan doesn't complain
...
TEST_F(ArrayTest, Serialization) {
	::rc::detail::checkGTest([=](int8_t plaintext_num) {
		auto input_arr = Array<Int8, 1>(true, params);
		auto elem = Int8(plaintext_num, params);
